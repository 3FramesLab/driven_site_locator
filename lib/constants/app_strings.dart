class AppStrings {
  static const fuelmanTapOnMapLocations =
      'Tap on the map to view Fuelman locations';
  static const comdataTapOnMapLocations =
      'Tap on the map to view Comdata locations';
  static const comdataWelcomeScreenInfo =
      'Driven for Comdata™ gives you complete access to your fuel cards and fleet management program.';
  static const fuelmanWelcomeScreenInfo =
      'View your account balance, pay your bill, view your purchase activity, and manage your cards quickly and easily from anywhere at anytime.';
  static const fuelmanLegalPrivacy = 'Legal & Privacy';
  static const bottomNavBarHeight = 0.0;
  static const trueText = 'true';
  static const falseText = 'false';
  static const noneText = 'none';
}

class WidgetKeys {
  static const editCardKey = 'edit_card_key';
}
