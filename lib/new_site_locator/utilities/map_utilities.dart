...
class MapUtilities {
      ...
      if (tapCount == SLInternalText.locationEnableDialogCount - 1) {
        if (Get.isDialogOpen ?? false) {
          await Future.delayed(const Duration(
            seconds: SLInternalText.noLocationsErrorModalTime + 1,
          ));
        }
  ...
  static void showLocationEnableDialog() {
    Get.dialog(
      const SingleFunctionDialog(
        dialogTitle: SLInternalText.locationEnableDialogTitle,
        buttonTitle: SLInternalText.locationEnableDialogButtonText,
  ...
  static double distanceFromCenter({
    ...
    switch (to) {
      case DeviceEdge.top:
        destination =
            LatLng(latLngBounds.northeast.latitude, centerFrom.longitude);
        break;
      case DeviceEdge.bottom:
        destination =
            LatLng(latLngBounds.southwest.latitude, centerFrom.longitude);
        break;
      case DeviceEdge.left:
        destination =
            LatLng(centerFrom.latitude, latLngBounds.southwest.longitude);
        break;
      case DeviceEdge.right:
        destination =
            LatLng(centerFrom.latitude, latLngBounds.northeast.longitude);
      case DeviceEdge.topRight:
        destination = latLngBounds.northeast;
        break;
    }

    // return distanceBetweenTwoLocation(centerFrom, destination); // in meters
    return distanceBetweenTwoLocation(centerFrom, destination) *
        0.000621371; // in miles
  }
}
